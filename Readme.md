...
7) Install drangonflybsd from the USB stick assumping the entire 2nd drive is used. Follow the directions here:
   ...
   ```
   Assumming the directions in the link above were followed. The _os_dragonflybsd.png_ file is the /dflybsd.ico file converted to png using Gimp.
   
   ![dflypowered1](https://github.com/catfacedck/Dragonflybsd-Acer-Nitro-Laptops-AN515-5158-XXX/assets/42676711/e96a69b9-0cfb-4cd6-8124-60b4611365af)
...
8) After installation, reboot the laptop from the drive not the USB stick. When the keys flash select F12 for the rEFind boot menu. Select dragonflybsd. Login as root and check that the /etc/fstab file looks something like this. The drive ID will be different matching dmesg.
   ...
   reboot
   ```
   When the keys flash select F12 for the rEFind boot menu. Select dragonflybsd.
...
11) Provision Xorg. Add the following content to /etc/sysctl.conf, /boot/loader.conf, and /etc/rc.conf .
    ...
    When the keys flash select F12 for the rEFind boot menu. Select dragonflybsd, boot and look for the trouble.
...
14) Miscellaneous.
Install additional development utilities. At the command prompt type:
```
pkg install usbutils
```
This installs _lsusb_ together with a few other utilities useful for developmen/debugging.
...
>
> <ins>Openbsd 7.4/7.5:</ins> supports WiFi (iwx) and Intel -P GT2 Iris Xe Graphics using drm. Nearly everything is functional (no camera) including iic mousepad, suspend/resume closing the display lid is functional, except key suspend/resume. Likely a configuration issue. No Oracle Java.
>
> <ins>Freebsd 14-stable/15-current:</ins> supports WiFi and Intel -P GT2 Iris Xe Graphics using drm-61-kmod. Wifi (iwlwifi) is problematic sometimes hanging the system and/or dropping connections. Freebsd 14-release (or less) does not support Intel WiFi or Graphics on this platform. Camera functional after patching webcamd usb/uvc. Suspend/resume is nonfunctional as is iic mousepad (use psm instead). No Oracle Java.
>
